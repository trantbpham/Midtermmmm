...
public class LinearProbingHashST<Key, Value> {
    ...
    public void delete(Key key) {
        ...
        // loop through to the key position[i]
        int i = hash(key);
        while (!key.equals(keys[i])) {
            i = (i + 1) % m;
        }

        // set key and its associated value to null for deletion
        keys[i] = null;
        vals[i] = null;

        // rehash the remaining keys
        i = (i + 1) % m;
        // Loop through again to reorder the remaining keys
        while (keys[i] != null) {
            // create temporary values to store keys & values for reassignment
            Key   keyToRehash = keys[i];
            Value valToRehash = vals[i];
            keys[i] = null;
            vals[i] = null;
            // shift the elements backwards
            n--;
            // reassign keys and values to the new index
            put(keyToRehash, valToRehash);
            //move to the next key+value for reassignment
            i = (i + 1) % m;
        }
